...
```{r}
rf_spec2 <- rand_forest(mtry = 6, trees = tune(), min_n = tune()) %>%
  set_engine("ranger", importance = "impurity") %>%
  set_mode("classification")
...
```
It appears as if trees over 1500 with minimal node sizes ranging from 0-25 seem to yield the best performance.

# Question 7
```{r}
arrange(collect_metrics(tune_res2))
best <- select_best(tune_res2, metric = "roc_auc")
best
```
The roc_auc of my best performing random forest model is .7427192.
